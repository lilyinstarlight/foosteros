...

{
  ...
  home-manager.users.lily = { pkgs, lib, ... }: let cfg = config.home-manager.users.lily; in {
    ...
    systemd.user.services.mpdris2 = {
      ...
      Service.ExecStartPre = "${pkgs.coreutils}/bin/timeout 60 ${pkgs.runtimeShell} -c 'while ! ${pkgs.iproute2}/sbin/ss -tlnH sport = :${toString cfg.services.mpdris2.mpd.port} | ${pkgs.gnugrep}/bin/grep -q \"^LISTEN.*:${toString cfg.services.mpdris2.mpd.port}\"; do ${pkgs.coreutils}/bin/sleep 1; done'";
    };
