...
lib.mkIf config.foosteros.profiles.sway {
  ...
  services.greetd.settings.default_session.command = let
    greetdSwayConfig = pkgs.writeText "greetd-sway-config" ''
      ...
      exec "${lib.getExe config.services.displayManager.regreet.package} >/var/log/regreet/stdio 2>&1; swaymsg exit"
    '';
  in "${lib.getExe (config.programs.sway.package.override { withGtkWrapper = false; })} --config ${greetdSwayConfig}";
