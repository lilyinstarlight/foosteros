...
DEFAULT_IN = ROOT.joinpath("fish-plugin-names")
...
GET_PLUGINS = f"""(with import <nixpkgs> {{}}; with import <localpkgs> {{ pkgs = nixpkgs; }};
let
  ...
  checksums = lib.listToAttrs (lib.forEach generated getChecksum);
...
class CleanEnvironment(object):
    def __enter__(self) -> None:
        ...
        local_pkgs = str(ROOT.joinpath("../../pkgs"))
        old_path = os.environ["NIX_PATH"]
        os.environ["NIX_PATH"] = f"localpkgs={local_pkgs}:{old_path}"
...
def get_cache_path() -> Optional[Path]:
    ...
    return Path(xdg_cache, "fish-plugin-cache.json")
...
header = "# This file has been generated by ./misc/fish-plugins/update.py. Do not edit!"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Updates home manager list for fish plugins"
    ...
    parser.add_argument(
        ...
        default=[],
        action="append",
        help="Plugin to add to misc/fish-plugins from Github in the form owner/repo",
    )
    parser.add_argument(
        "--input-names",
        "-i",
...
def commit(repo: git.Repo, message: str, files: List[Path]) -> None:
    ...
    if files_staged:
        print(f'committing to foosteros "{message}"')
...
def main():
    ...
    commit(nixpkgs_repo, "misc/fish-plugins: update", [args.outfile])

    if redirects:
        update()
        commit(
            nixpkgs_repo,
            "misc/fish-plugins: resolve github repository redirects",
    ...
    for plugin_line in args.add_plugins:
        ...
        commit(
            nixpkgs_repo,
            "misc/fish-plugins: init {name} at {version}".format(
